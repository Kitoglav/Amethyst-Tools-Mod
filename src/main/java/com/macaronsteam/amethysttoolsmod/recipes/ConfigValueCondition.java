...
public class ConfigValueCondition implements ICondition {
  private static final ResourceLocation NAME =
      new ResourceLocation(AmethystToolsMod.MODID, "config_value");
  ...
  public static class Serializer implements IConditionSerializer<ConfigValueCondition> {
    ...
    public void write(JsonObject json, ConfigValueCondition value) {
      json.addProperty("configValue",
          value.value.getPath().stream().collect(Collectors.joining(".")));
    }

    @Override
    public ConfigValueCondition read(JsonObject json) {
      return new ConfigValueCondition(
          AmethystToolsModConfig.spec.getValues().get(GsonHelper.getAsString(json, "configValue")));
    }
